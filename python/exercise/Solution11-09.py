...
def clickListBox(evt):
    ...
    if  dirName == '상위폴더' :  
        if len(searchDirList) == 1 :  # 상위 폴더를 클릭했는데, 현재 C:\\ 면
            return
...
def  fillListBox() :  # 항상 제일 검색한 폴더 리스트의 마지막 폴더(=현재 폴더)를 표시 
    ...
    for item in folderList:
        if os.path.isdir(currentDir+item) :
            ...
            # 파일 크기 추가
            fSize = os.path.getsize(currentDir+item)
            fSize = int(fSize /1024)
            if fSize >= 1024 :
                fSize = int(fSize / 1024)
                fileListBox.insert(END, item+'     [' + str(fSize) + ' MB]')
            else :
                fileListBox.insert(END, item+'     [' + str(fSize) + ' KB]')
            # 리스트 박스의 마지막 추가한 것에 대한 색상 표시
            listboxSize = fileListBox.size()
            fName, extName = os.path.splitext(item)
            if extName in ['.exe', '.msi', '.com', '.bat'] :
                fileListBox.itemconfig(listboxSize-1, foreground='green')
            elif extName in ['.jpg', '.bmp', '.png', '.gif'] :
                fileListBox.itemconfig(listboxSize-1, foreground='red')
            elif extName in ['.py'] :
                 fileListBox.itemconfig(listboxSize-1, foreground='blue')                
...
if __name__ == "__main__" :
    window=Tk()
    window.title("폴더 및 파일 목록 보기(기능 추가)")
