...
if __name__ == "__main__" :
    window=Tk()
    window.title("객체지향 그림판(개선) ")
    canvas = Canvas(window, height=300, width=300)
    
    canvas.bind("<Double-Button-1>", lambda event: deleteCircle(event,'Circle'))
    canvas.bind("<Double-Button-2>", lambda event: deleteRectangle(event,'Rectangle'))
